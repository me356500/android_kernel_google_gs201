...
#include <linux/list.h> // wyc add
#include <linux/migrate_swap.h> // tyc add
...
DEFINE_HASHTABLE(pid_to_swap_offset_hash, 13); // ycc add
#ifdef swap_alloc_enable
static DEFINE_SPINLOCK(swap_alloc_lock);
static struct pid_swap_map_node *get_pid_hash(int pid)
{
	struct pid_swap_map_node *pid_slot;
	// int pid;
	// pid = -1;
	// if (vma && vma->vm_mm && vma->vm_mm->owner)
	// 	pid = vma->vm_mm->owner->pid;

	hash_for_each_possible (pid_to_swap_offset_hash, pid_slot, hash, pid)
		if (pid == pid_slot->pid)
			return pid_slot;

	return NULL;
}
static unsigned swap_alloc_free_offset = 1;
// wyc add
...
static int scan_swap_map_slots(struct swap_info_struct *si,
			       ...
			       swp_entry_t slots[], struct vm_area_struct *vma)
{
	struct swap_cluster_info *ci;
	unsigned long offset;
	...
	// 		pid_swap_map->offest = 0;
	// 		pid_swap_map->partition = 0;
	// 		hash_add(pid_to_swap_offset_hash, &pid_swap_map->hash, page_pid);
	// 		printk("ycc pid NULL %d", page_pid);
	// 	} else {
	// 		pid_swap_map->offest++;
	// 		pid_swap_map->partition++;
	// 		printk("ycc pid find partiton %d offset %d pid %d", pid_swap_map->offest,
	// 		       pid_swap_map->partition, page_pid);
	// 	}
...
static void swap_compaction(struct swap_info_struct *si, unsigned long type) {
    ...
    unsigned int victim_cnt = 256;
	unsigned int offset = 0;
	unsigned char swap_map;
    struct free_swap_map_node *free_node;
	struct pid_swap_map_node *pid_swap_map;
	int page_pid = 0;
    
    // victim selection : greedy
    for (block_id = 0; block_id < flash_swap_block - 1; block_id++) {
        block_cnt = prnt_cnt = 0;

		// skip empty block & open block
		page_pid = swap_block_pid[block_id];
		if (page_pid == 0) {
			continue;
		}

		pid_swap_map = get_pid_hash(page_pid);
		if (((pid_swap_map->offest - 1) / per_app_swap_slot) == block_id) {
			continue;
		}

        for (block_offset = 1; block_offset <= per_app_swap_slot; block_offset++) {
			
			offset = (block_id * per_app_swap_slot) + block_offset;
			swap_map = READ_ONCE(si->swap_map[offset]);
            if (swap_map != SWAP_MAP_BAD && swap_map && swap_map != SWAP_HAS_CACHE) {
                ++block_cnt;
            }
			if (swap_map) {
				++prnt_cnt;
	...
	// Record migrate statistics
	total_valid_slot += victim_cnt;
	total_compaction_cnt += 1;

	if (victim_cnt < min_valid_slot)
		min_valid_slot = victim_cnt;
	
	if (victim_cnt > max_valid_slot)
		max_valid_slot = victim_cnt;
    ...
    list_add_tail(&free_node->list, &free_blk_list);
	++free_blk_cnt;
	spin_unlock(&free_list_lock);

	// reset pid map
	spin_lock(&swap_block_pid_lock);
	swap_block_pid[victim_id] = 0;
...
pid_to_hash:
	...
	if (page_pid != -1) {
		...
		if (pid_swap_map == NULL) {
			...
			//        pid_swap_map->used_page, page_pid, swap_alloc_free_offset, READ_ONCE(si->highest_bit));
		}
		// wyc record swap block pid
		spin_lock(&swap_block_pid_lock);
		swap_block_pid[((offset - 1) / per_app_swap_slot)] = page_pid;
		spin_unlock(&swap_block_pid_lock);
		if ((offset) % 100000 == 0)
			printk("wyc bid: %ld, offset: %ld, pid: %d\n", ((offset - 1)/ per_app_swap_slot), offset, page_pid);
...
checks:
	...
	/*
	if ((swap_first_full && page_pid != -1) && !skip_app_hole) { // swap full
		for (block_id = 0; block_id < 4096; block_id++) {
		...
		// no free slot
		if (block_id >= 4096) {
			//printk("wyc no_free_slot for pid: %d\n", page_pid);
			skip_app_hole = 1;
			scan_base = offset = si->lowest_bit;
		}
		else  {
...
scan_swap_alloc:
	spin_unlock(&si->lock);
	while (offset < scan_base + 256) { // scan_base + 256   READ_ONCE(si->highest_bit)
		if (data_race(!si->swap_map[offset])) {
			spin_lock(&si->lock);
			goto checks;
		}
		...
		// printk("ycc swap alloc offset used start-offset %d %d, %d", start_offset, offset, used_page);
		offset++;
	}
	spin_lock(&si->lock);
	// printk("ycc swap alloc not match start-offset %d, %d", start_offset, used_page);
	if (page_pid != -1)
	{
		spin_lock(&swap_alloc_lock);
				   ...
				   unsigned char usage)
{
	...
	n_ret = scan_swap_map_slots(si, usage, 1, &entry, NULL);
...
int get_swap_pages(int n_goal, swp_entry_t swp_entries[], int entry_size, int reswp, struct vm_area_struct *vma) // ycc modify
{
...
start_over:
	...
	plist_for_each_entry_safe(si, next, &swap_avail_heads[node], avail_lists[node]) {
		...
		if(!si->type) // ycc only do swap allocation on flash
			vma=NULL;
...
#else
			n_ret = scan_swap_map_slots(si, SWAP_HAS_CACHE, n_goal, swp_entries, vma);
#endif
		}
		spin_unlock(&si->lock);
