...
static void swap_compaction(struct swap_info_struct *si, unsigned long type) {
    ...
    for (block_id = 0; block_id < flash_swap_block - 1; block_id++) {
		...
		pid_swap_map = get_pid_hash(page_pid);
		if (((pid_swap_map->offest - 1) / per_app_swap_slot) == block_id && pid_swap_map->used_page < per_app_swap_slot) {
			continue;
		}
			       ...
			       swp_entry_t slots[], struct vm_area_struct *vma) // ycc flash swap alloc
{
	...
	if (!first_full && list_empty(&free_blk_list) && !free_list_init) {
		...
		free_list_init = 1;
		printk("wyc branch_same_app_full_block\n");
	}
	spin_unlock(&free_list_lock);
...
checks:
	...
	if (free_list_init && !free_blk_cnt) { // swap full
		...
		for (block_id = 0; block_id < 4085; block_id++) {
			...
			if (block_cnt && block_cnt < victim_cnt) {
				victim_cnt = block_cnt;
				victim_id = block_id;
			}
		}
